...
# Create Errorlist
if(use.ifs == TRUE) {
  #NOTE: COMMENT IS MOSTLY PLACEHOLDER
  #commandArgs = function() c(FALSE, "filename")
  #args = commandArgs()
  #userFile = commandArgs
  #req = read.csv(userFile)
  #  userFile = basename(userFile)
  userFile = "xgongivittoya.csv" #this userFile is placeholder!
  #for json, output file should be named "feedback_cis3750req_", then cat with given filename (include .txt extension)
  fileName = paste("Results/feedback_cis3750req_", userFile, ".txt", sep="")
...
if(use.ifs == TRUE) {
  json.output = read.delim(fileName, header = FALSE)
  json.output$V1[nrow(json.output)] = ""
  cat(as.character(json.output$V1), "}]", "\n", "}", "\n", file = file.open, sep = "", append = FALSE)
}
